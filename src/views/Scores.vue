<template>
    <v-layout class="mt-5 justify-center" row>
        <v-card class="ma-10 w-25 pa-2" elevation="10"  >
            <v-card-title >
                <h1 style="margin: 10px;"><b>Quiz History</b></h1> </v-card-title>
                
           
            <v-list :class="{ active: index == currentIndex }" v-for="(history, index) in List" :key="index">
             
                <v-card color="indigo darken-2">
                    <v-list-item color="teal lighten-5" @click="setActiveHistory(history, index)">{{index+1+" User ID: " + history.userID  
                    }}
                    </v-list-item>
                </v-card >
            </v-list>
        ...
        <v-card class="ma-10 w-50" elevation="10" color="teal lighten-5">
            <v-card-title>
